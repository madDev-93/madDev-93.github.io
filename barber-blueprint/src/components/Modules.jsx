...
import { usePublicModules } from '../hooks/useModules'

// Get icon component from string name
function getIcon(iconName) {
  return LucideIcons[iconName] || BookOpen
}
...
export default function Modules() {
  ...
  const { modules: firestoreModules, loading } = usePublicModules()
  const modules = firestoreModules.length > 0 ? firestoreModules : fallbackModules
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 id="modules-heading" className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 tracking-tight">
            {modules.length} Modules to Transform Your Game
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6" role="list" aria-label="Course modules">
          {modules.map((module, index) => {
            // Handle both Firestore (icon as string) and fallback (icon as component)
            const IconComponent = typeof module.icon === 'string' ? getIcon(module.icon) : module.icon
            return (
              <motion.article
              ...
              >
                <TiltCard className="group relative bg-dark-tertiary border border-white/5 rounded-2xl p-6 hover:border-gold/20 transition-all duration-300 h-full">
                  ...
                  <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center mb-4 group-hover:bg-gold/10 transition-colors" aria-hidden="true">
                    <IconComponent className="w-6 h-6 text-gray-400 group-hover:text-gold transition-colors" />
                  </div>
